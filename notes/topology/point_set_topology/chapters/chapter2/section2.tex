...
\begin{example}
    \begin{enumerate}
        ...
        \item[(3)] Define $d:\R^n \times \R^n \rightarrow \R$ by
            ...
            \begin{figure}[h]
                \centering
                \includegraphics[scale=0.5]{Figures/chapter2/open_ball_taxicab.eps}
                \caption{The open ball of radius $1$ centered about the point
                $0 \times 0$ in the taxicab metric}
                \label{fig_2.2}
            \end{figure}

        \item[(4)] For the metric $d'\R^n \times \R^n \rightarrow \R$ defined by
            \begin{equation*}
                d'(x,y)=\sqrt[p]{\sum_{i=1}^n{|x_i-y_i|^p}}
            \end{equation*}
            for some $p$ also induces the usual topology on $\R^n$. For if  $y
            \in B(x, \epsilon)$, then
            $d'(x,y)=\sum_{i=1}^n{|x_i-y_i|^p}<\epsilon^p$, which makes
            $|x_i-y_i|^p<\epsilon^p$, so that $|x_i-y_i|<\epsilon$, which is the
...
\begin{proof}
    ...
    0$, and  if $||x-y||=0$ then  $(x_1-y_1)^2+ \dots +(x_n-y_n)^2=0$ hence
    $x_i=y_i$ for all  $1 \leq i \leq n$, and if  $x_i=y_i$, then clearly
    $||x-y||=0$. We also see that $||x-y||=||y-x||$.
    ...
    if and only if $x=y$  (since $|\cdot|$ is also a metric), we also see that $\rho(x,y)=\rho(y,x)$.
...
\begin{lemma}\label{2.2.7}
    ...
    $\epsilon>0$, there is a  $\delta>0$ such that  $B_{d'}(x,\delta)
